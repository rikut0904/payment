...
const { parseAmount, parseBillingDay } = require('./numberUtils');
...
function buildCardPayload(
  ...
  const payload = {
    ...
    closingDay: parseBillingDay(closingDay),
    paymentDay: parseBillingDay(paymentDay),
...
function buildSubscriptionPayload(
  { userId, cardId, serviceName, amount, currency, notes, cycle, registeredEmail, paymentStartDate },
  { includeUserId = false } = {}
) {
  // サブスク保存用のペイロードを作成する。
...
async function updateSubscription({
  ...
  const payload = Object.assign(
    {},
    buildSubscriptionPayload({ userId, cardId, serviceName, amount, currency, notes, cycle, registeredEmail, paymentStartDate }),
    { updatedAt: admin.firestore.FieldValue.serverTimestamp() }
  );
  await docRef.update(payload);
}

async function updateSubscriptionCard({ id, userId, cardId }) {
  // サブスクの紐づけカードのみ更新する。
  if (!id) throw new Error('subscriptionId is required');
  if (!userId) throw new Error('userId is required');
  if (!cardId) throw new Error('cardId is required');
