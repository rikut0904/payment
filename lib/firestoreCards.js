...
async function updateCard({
  ...
  if (!userId) throw new Error('userId is required');
  if (!cardName) throw new Error('cardName is required');
  const docRef = getDb().collection('cards').doc(id);
...
async function updateSubscription({
  ...
  if (!userId) throw new Error('userId is required');
  const docRef = getDb().collection('subscriptions').doc(id);
  const snapshot = await docRef.get();
  if (!snapshot.exists) {
...
async function deleteSubscription(subscriptionId, userId) {
  ...
  if (!userId) throw new Error('userId is required');
  const docRef = getDb().collection('subscriptions').doc(subscriptionId);
  const snapshot = await docRef.get();
  if (!snapshot.exists) {
    throw new Error('subscription not found');
  }
