...
async function updateUserNameForUser(userId, userName) {
  ...
  for (let i = 0; i < docs.length; i += chunkSize) {
    ...
    await batch.commit();
    updatedCount += Math.min(chunkSize, docs.length - i);
  }
  return updatedCount;
}
...
async function deleteLikesByUser(userId) {
  ...
  for (let i = 0; i < docs.length; i += chunkSize) {
    ...
    await batch.commit();
    deletedCount += Math.min(chunkSize, docs.length - i);
  }
  return deletedCount;
}
